...
import com.utilities.Loaders.{compute, getEmbeddings, getEmbeddingsinLSHFormat, getExactNeighbors, getNeighbors, getTestData, getTrainData}
...
object Prediction {
  ...
  def main(args: Array[String]) {
    ...
    parser.parse(args, defaultParams).map{
      ...
      //      val testData = getTestData(param.test)
        ...
        if (neighborType == "LSH") {
          ...
          predictions._1.coalesce(1).saveAsTextFile(param.predictions) // ._1
        }
        else {
